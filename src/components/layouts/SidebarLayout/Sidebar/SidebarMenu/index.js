...
function SidebarMenu() {
  ...
  const { user } = useAuth();
  const { role } = JSON.parse(user.reloadUserInfo.customAttributes);
  ...
  const RoleWrapper = ({ children, role, section }) => {
    if (section.allow.includes(role)) {
      return <>{children}</>;
    }
  };
  ...
  return (
    <>
      {menuItems.map((section) => (
        <RoleWrapper role={role} section={section}>
